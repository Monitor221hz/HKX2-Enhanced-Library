...
public sealed class hkpConstraintAtomUnionFormatter : MemoryPackFormatter<hkpConstraintAtom>
{
    public override void Serialize<TBufferWriter>(
        ref MemoryPackWriter<TBufferWriter> writer, scoped ref hkpConstraintAtom? value)
    {
        if (value == null)
        {
        ...
        switch (value)
        {
            // most-derived types MUST come before their base types in the switch
            ...
            case hkpAngConstraintAtom v6: writer.WriteUnionHeader(6); writer.WritePackable(v6); break;
            case hkpLinConstraintAtom v7: writer.WriteUnionHeader(7); writer.WritePackable(v7); break;
            default: throw new InvalidOperationException($"Unknown type for hkpConstraintAtom union: {value.GetType().Name}");
        }
    }

    public override void Deserialize(ref MemoryPackReader reader, scoped ref hkpConstraintAtom? value)
    {
        ...
        switch (tag)
        {
            ...
            case 6: { var v = default(hkpAngConstraintAtom); reader.ReadPackable(ref v); value = v; break; }
            case 7: { var v = default(hkpLinConstraintAtom); reader.ReadPackable(ref v); value = v; break; }
            default: throw new InvalidOperationException($"Unknown tag {tag} for hkpConstraintAtom union");
        }
    }
}
