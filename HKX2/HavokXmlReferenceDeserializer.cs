...
namespace HKX2E
{
    public class HavokXmlReferenceDeserializer : HavokXmlDeserializer
    {
        ...
        public int GetOrder(string name) =>
            nameOrderLookup.TryGetValue(name, out var index) ? index : 0;
        ...
        public virtual IHavokObject Deserialize(
            Stream stream,
        ...
        public override T? ReadClassPointer<T>(IHavokObject owner, XElement element, string name)
            where T : default
        {
        ...
        public override IList<T> ReadClassPointerArray<T>(
            IHavokObject owner,
            XElement element,
            string name
        )
            where T : default
        {
            ...
            for (int i = 0; i < refNames.Length; i++)
            {
                ...
                if (!elementNameMap.TryGetValue(refName, out XElement? refEle))
                    throw new Exception(
                        $"Reference symbol '{refName}' not found. Make sure it defined somewhere. at Line {((IXmlLineInfo)element)?.LineNumber ?? -1}, Property: {name}"
                    );
        ...
        public override T?[] ReadClassPointerCStyleArray<T>(
            IHavokObject owner,
            XElement element,
            string name,
            short length
