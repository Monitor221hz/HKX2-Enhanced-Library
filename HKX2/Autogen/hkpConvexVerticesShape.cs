...
namespace HKX2E
{
    ...
    public partial class hkpConvexVerticesShape
        : hkpConvexShape,
            IEquatable<hkpConvexVerticesShape?>
    {
        public Vector4 aabbHalfExtents { set; get; }
        public Vector4 aabbCenter { set; get; }
        public IList<hkpConvexVerticesShapeFourVectors> rotatedVertices { set; get; } =
            Array.Empty<hkpConvexVerticesShapeFourVectors>();
        ...
        public override void ReadXml(IHavokXmlReader xd, XElement xe)
        {
            ...
            rotatedVertices = xd.ReadClassArray<hkpConvexVerticesShapeFourVectors>(
                xe,
                nameof(rotatedVertices)
            );
            numVertices = xd.ReadInt32(xe, nameof(numVertices));
            planeEquations = xd.ReadVector4Array(xe, nameof(planeEquations));
            connectivity = xd.ReadClassPointer<hkpConvexVerticesConnectivity>(
                this,
                xe,
                nameof(connectivity)
            );
        ...
        public bool Equals(hkpConvexVerticesShape? other)
        {
            return other is not null
                ...
                && (
                    (connectivity is null && other.connectivity is null)
                    || (
                        connectivity is not null
                        && other.connectivity is not null
                        && connectivity.Equals((IHavokObject)other.connectivity)
                    )
                )
